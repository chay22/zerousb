...
// Copyright 2016 the zerousb Authors.  All rights reserved.
//
...
func ParseIDs(r io.Reader) (map[uint16]*Vendor, map[uint8]*Class, error) {
	vendors := make(map[uint16]*Vendor, 2800)
	...
	parseVendor := func(level int, raw uint64, name string) error {
		...
		case 1:
			...
			if vendor.Product == nil {
				vendor.Product = make(map[uint16]*Product)
			}
		...
		case 2:
			...
			if device.Interface == nil {
				device.Interface = make(map[uint16]string)
			}
	...
	parseClass := func(level int, id uint64, name string) error {
		...
		case 1:
			...
			class.SubClass[uint8(id)] = subclass
